...
function rawPromisify(obj) {
    lodash.each(obj, function (method, name) {
...
function pruned(obj) {
    return lodash.pick(obj, cint.not(lodash.isUndefined))
}
...
function versionNumParts(version) {
    ...
    return lodash.intersection(VERSION_PARTS, Object.keys(semver)).length;
}
...
function precisionAdd(precision, n) {
    ...
    var index = n === 0 ? precision :
        lodash.contains(VERSION_BASE_PARTS, precision) ? VERSION_BASE_PARTS.indexOf(precision) + n :
        _.contains(VERSIONlodashADDED_PARTS, precision) ? VERSION_BASE_PARTS.length + n :
        null;
...
function semverBuildVersionString(semver, precision) {
    ...
    return parts
        .filter(function(part) {
            return lodash.contains(VERSION_BASE_PARTS, precision) || semver[part];
        })
        .map(function(part) {
...
function getVersionPrecision(version) {
    ...
    return lodash.find(VERSION_PARTS.slice().reverse(), lodash.propertyOf(semver));
}
...
function upgradeDependencyDeclaration(declaration, latestVersion, options) {
    ...
    var parsedRange = lodash(semverutils.parseRange(declaration))
    ...
    var uniqueOperators = lodash(parsedRange)
        .pluck('operator')
        .uniq()
        .value();
    var operator = uniqueOperators[0] || '';

    var hasWildCard = WILDCARDS.some(lodash.partial(lodash.contains, newSemverString, lodash, 0));
...
function upgradeDependencies(currentDependencies, latestVersions) {
    ...
    var upgradeDep = lodash.partialRight(upgradeDependencyDeclaration, {
        wildcard: wildcard
    });

    return lodash(currentDependencies)
        ...
        .pick(lodash.spread(isUpgradeable))
        .mapValues(lodash.spread(upgradeDep))
        .value();
}
...
function packageNameFilter(filter) {
    ...
    // no filter
    if(!filter) {
        filterPackages = lodash.identity;
    }
    // RegExp filter
    ...
    // string filter
        ...
        filterPackages = lodash.contains.bind(lodash, packages);
    }
    // array filter
    else if(Array.isArray(filter)) {
        filterPackages = lodash.contains.bind(lodash, filter);
    }
    else {
...
function getCurrentDependencies(pkg, options) {
    ...
    var allDependencies = cint.filterObject(lodash.merge({},
...
function npmView(packageName, field) {
    ...
    return npm.commands.viewAsync([packageName, field], true)
        .then(function (response) {
            return lodash.values(response)[0][field];
        });
}
...
function getGreatestPackageVersion(packages) {
    return getPackageVersions(packages).then(lodash.last);
}
...
function getLatestVersions(packageList, options) {
    ...
    // TODO: It would be better here to create an object of promised versions, keyed by the package name, instead of an array. This should make the final lodash.pick unnecessary and subsequently allow the whole PromiseInspection logic to be generalized
    ...
    return Promise.settle(packageList.map(getPackageVersion))
        ...
        .then(lodash.partialRight(lodash.pick, lodash.identity))
}
...
function getPreferredWildcard(dependencies) {
    ...
    var groups = lodash.groupBy(lodash.values(dependencies), function(dep) {
        return lodash.find(WILDCARDS, function(wildcard) {
    ...
    var sorted = lodash.sortBy(arrOfGroups, function(wildcardObject) {
