...
function updatePackageData(pkgData, oldDependencies, newDependencies) {
    for (var dependency in newDependencies) {
        if (newDependencies.hasOwnProperty(dependency)) {
...
function getInstalledPackages() {
    return npm.commands.listAsync([], true)
        .then(function (results) {
            ...
            var globalPackages = {};
            for (var _package in packageList) {
                if (packageList.hasOwnProperty(_package)) {
                    if (packageList[_package] !== "*") {
                        globalPackages[packageList[_package].name] = packageList[_package].version;
                    }
                }
            }
