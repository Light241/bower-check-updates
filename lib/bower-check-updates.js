...
function printUpgrades(args) {
    ...
    if (numSatisfied > 0) {
        ...
        print((numUnsatisfied > 0 ? '\n' : '') + 'The following dependenc' + (numSatisfied === 1 ? 'y is' : 'ies are') + ' satisfied by ' + (numSatisfied === 1 ? 'its' : 'their') + ' declared version range, but the installed version' + (numSatisfied === 1 ? ' is' : 's are') + ' behind. You can install the latest version' + (numSatisfied === 1 ? '' : 's') + ' without modifying your package.json by using ' + chalk.blue('bower update') + '. If you want to update the dependenc' + (numSatisfied === 1 ? 'y' : 'ies') + ' in your bower.json anyway, use ' + chalk.blue('bcu --upgradeAll') + '.\n');
...
function programInit() {
    ...
    if (options.global && options.upgrade) {
        programError(chalk.blue('bcu') + ' cannot upgrade global packages. Run ' + chalk.blue('bower install -g [package]') +
...
function programRunLocal() {
    ...
    // no package data!
    if (!pkgData) {
        programError(chalk.red('No bower.json') + '\n\nPlease add a bower.json to the current directory, specify the ' + chalk.blue('--packageFile') + ' or ' + chalk.blue('--packageData') + ' options, or pipe a bower.json to stdin.');
    }
