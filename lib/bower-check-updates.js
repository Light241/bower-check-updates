...
//TODO (S.Panfilov) read-package-json?
...
var print = program.silent ? lodash.noop : console.log;
var readBowerFile = cint.partialAt(fs.readFileAsync, 1, 'utf8');
var writeBowerFile = fs.writeFileAsync;

//
// Main functions
//

function upgradeBowerDefinitions(currentDependencies) {
...
function analyzeGlobalBower() {
    return vm.getInstalledPackages()
        .then(function (globalBower) {
            return upgradeBowerDefinitions(globalBower)
                .spread(function (upgraded) {
                    printGlobalUpgrades(globalBower, upgraded)
                });
        });
}

function analyzeProjectDependencies(pkgData, pkgFile) {
    var options = lodash.pick(program, ['filter', 'prod', 'dev']);
    ...
    return Promise.all([
        ...
        upgradeBowerDefinitions(current)
    ])
        ...
        .spread(function (current, installed, upgraded, latest) {
            ...
            if (program.json) {
                ...
                print(program.jsonAll ? JSON.parse(newPkgData) :
                        program.jsonDeps ? lodash.pick(JSON.parse(newPkgData), 'dependencies', 'devDependencies') :
                            upgraded
                );
            }
            else {
                ...
                if (pkgFile && !lodash.isEmpty(upgraded)) {
                    if (program.upgrade) {
                        newPkgData = vm.updatePackageData(pkgData, current, upgraded);
                        writeBowerFile(pkgFile, newPkgData)
                            .then(function () {
                                print('\n' + pkgFile + " upgraded");
                            });
                    } else {
...
function programInit() {
    ...
    if (program.global && program.upgrade) {
        ...
        print("Run 'npm install -g [bower.json]' to upgrade a global bower.json.");
        process.exit(1);
    }

    // add shortcut for any keys that start with 'json'
    program.json = lodash(program)
        .keys()
        .filter(lodash.partial(lodash.startsWith, lodash, 'json', 0))
        .some(lodash.propertyOf(program));
}
...
function programRunLocal() {
    ...
    if (!process.stdin.isTTY) {
        ...
        if (program.args[0] && fs.existsSync(program.args[0])) {
            ...
            bowerInSameDir = false;
        }

        var relPathToBower = path.relative(process.cwd(), pkgFile);
        ...
        if (relPathToBower !== 'bower.json') {
            print('Using ' + relPathToBower);
        }

        pkgData = readBowerFile(pkgFile, null, false);
    }

    return pkgData.then(lodash.partialRight(analyzeProjectDependencies, lodash, pkgFile));
}
